...
def test_infer_schema_empty_list() -> None:
    ...
    schema = infer([], return_model=False).schema_dict
...
def test_infer_schema_single_row() -> None:
    ...
    schema = infer([{"a": 1, "b": "x"}], return_model=False).schema_dict
    assert schema is not None
...
def test_infer_schema_single_row_with_none() -> None:
    ...
    schema = infer([{"id": 1, "name": None}], return_model=False).schema_dict
    assert schema is not None
...
def test_infer_schema_all_none_in_column() -> None:
    ...
    schema = infer([{"x": None}, {"x": None}], return_model=False).schema_dict
    assert schema is not None
...
def test_inferred_model_validates_matching_row() -> None:
    ...
    Model = infer(data, model_name="User").model
...
def test_schema_from_dict_raises_on_invalid_schema() -> None:
    from infermodel.schema import schema_from_dict

    with pytest.raises(ValueError, match="type.*model"):
        schema_from_dict({"schema_version": 1, "type": "list"})
    with pytest.raises(ValueError, match="type.*model"):
        schema_from_dict({"schema_version": 1})
...
def test_string_zero_inferred_as_int() -> None:
    ...
    schema = infer([{"n": "0"}, {"n": "0"}], return_model=False).schema_dict
    assert schema is not None
    assert schema["fields"]["n"]["type"] == "int"
...
def test_string_negative_inferred_as_int() -> None:
    ...
    schema = infer([{"n": "-1"}, {"n": "0"}], return_model=False).schema_dict
    assert schema is not None
    assert schema["fields"]["n"]["type"] == "int"
...
def test_string_whitespace_trimmed_for_number() -> None:
    ...
    schema = infer([{"n": "  42  "}, {"n": " 100 "}], return_model=False).schema_dict
    assert schema is not None
    assert schema["fields"]["n"]["type"] == "int"
...
def test_empty_string_stays_str() -> None:
    ...
    schema = infer([{"x": ""}, {"x": ""}], return_model=False).schema_dict
    assert schema is not None
...
def test_int_and_float_strings_promote_to_float() -> None:
    ...
    schema = infer([{"x": "1"}, {"x": "2.5"}], return_model=False).schema_dict
    assert schema is not None
...
def test_native_bool_inferred_as_bool() -> None:
    ...
    schema = infer([{"flag": True}, {"flag": False}], return_model=False).schema_dict
    assert schema is not None
...
def test_native_int_float_inferred() -> None:
    ...
    schema = infer([{"i": 1, "f": 1.5}], return_model=False).schema_dict
    assert schema is not None
...
def test_infer_schema_sample_size_zero_means_no_limit() -> None:
    ...
    def rows():
        ...
        yield {"x": 2}
        yield {"x": 3}
    schema = infer(rows(), config=InferConfig(sample_size=0), return_model=False).schema_dict
    assert schema is not None
...
def test_infer_schema_large_sample_size_is_ok() -> None:
    ...
    schema = infer(
        [{"x": 1}, {"x": 2}],
        config=InferConfig(sample_size=10_000_000),
        return_model=False,
    ).schema_dict
    assert schema is not None
    assert schema["fields"]["x"]["type"] == "int"
...
def test_numeric_promotion_strict_yields_any_on_int_float_mix() -> None:
    ...
    data = [{"x": 1}, {"x": 1.5}]
    schema = infer(
        data, config=InferConfig(numeric_promotion="strict"), return_model=False
    ).schema_dict
    assert schema is not None
    assert schema["fields"]["x"]["type"] == "any"
...
def test_inferred_nested_model_validates() -> None:
    ...
    Model = infer(data, model_name="UserWithNested").model
    assert Model is not None
...
def test_infer_model_multi_level_nested_dict_validates() -> None:
    ...
    Model = infer(data, model_name="Deep").model
    assert Model is not None
...
def test_nested_required_vs_nullable_semantics() -> None:
    ...
    Model = infer(data, model_name="NestedSemantics").model
    assert Model is not None
...
def test_dict_mixed_policy_union_produces_union_type() -> None:
    ...
    schema = infer(
        data, config=InferConfig(dict_mixed_policy="union"), return_model=False
    ).schema_dict
    assert schema is not None
...
def test_dict_mixed_policy_error_raises_value_error() -> None:
    ...
    with pytest.raises(ValueError):
        infer(data, config=InferConfig(dict_mixed_policy="error"))
...
def test_invalid_policy_value_raises_type_error() -> None:
    ...
    data = [{"x": 1}]
    with pytest.raises(TypeError):
        infer(data, config=InferConfig(dict_mixed_policy="bogus"))  # type: ignore[arg-type]


def test_infer_return_diagnostics_smoke() -> None:
    ...
    assert res.diagnostics["rows_used"] == 2
